...
def main(path: str) -> int:
    ...
    if sec6 is None:
        ...
        fails.append(
            "раздел 6 существует, но пуст. Заполняется 6.1 (что намеренно не делаем — "
            "известно при создании); 6.2 при создании пуст по построению, отвергать "
            "ещё нечего. Пересказ инвариантов §3 заполнением не считается"
        )
