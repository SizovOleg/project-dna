...
тот же промпт — и перекрыты **все** каналы доставки скилла, не только файл.
Замер 2026-08-18 (T-DISC-1): скилл, установленный в аккаунт, доставляется
субагентам по имени через Skill tool независимо от файловой системы — 13 из 18
«контрольных» агентов одного прогона получили полный текст протокола из облака.
Контроль без другого аккаунта или временного снятия скилла из аккаунта
неосуществим.
...
## Доставка протокола и пересказ в CLAUDE.md

Замер T-DISC-1: когда `CLAUDE.md` ссылается на протокол по имени и не
раскрывает его («работай по протоколу architect-cc-workflow, фазу закрывай
отчётом по формату протокола»), агенты вызывают скилл сами — 6 из 6 — и
применяют: останавливаются вместо фабрикации данных, производят отчёт в
формате семи секций. Когда строка в `CLAUDE.md` пересказывает правила,
протокол открывают 6 из 36: пересказ выключает чтение первоисточника.

Правило: **CLAUDE.md называет протокол, не пересказывает его.** Пересказ
допустим только для правил, которые обязаны сработать до любого чтения
(мандат фазы, запрет на конкретные файлы).
